...
class PAM(Layer):
    ...
    def call(self, input):
        ...
        vec_b = K.reshape(b, (-1, d1* h * w, filters // 8))
        vec_cT = tf.transpose(K.reshape(c, (-1, d1* h * w, filters // 8)), (0, 2, 1))
        bcT = K.batch_dot(vec_b, vec_cT)
        softmax_bcT = Activation('softmax')(bcT)
        vec_d = K.reshape(d, (-1, d1* h * w, filters))
        bcTd = K.batch_dot(softmax_bcT, vec_d)
        bcTd = K.reshape(bcTd, (-1, d1, h, w, filters))
